      ...
      <form onSubmit={save}>
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="card p-6">
            <h2 className="text-sm font-semibold text-[#0C0E12] mb-5 pb-4 border-b border-[rgba(12,14,18,0.06)] dark:border-white/10 font-display tracking-tight">Profil établissement</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-[#3A3D45] dark:text-[#9CA3AF] mb-1.5">Nom de l'établissement</label>
                <input className="input" value={form.name ?? ''} onChange={e => upd('name', e.target.value)} required />
              </div>
              <div>
                <label className="block text-xs font-medium text-[#3A3D45] dark:text-[#9CA3AF] mb-1.5">Secteur d'activité</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-[#3A3D45] dark:text-[#9CA3AF] mb-1.5">Ville</label>
              ...
              <div>
                <label className="block text-xs font-medium text-[#3A3D45] dark:text-[#9CA3AF] mb-1.5">Email professionnel</label>
              ...
              <div>
                <label className="block text-xs font-medium text-[#3A3D45] dark:text-[#9CA3AF] mb-1.5">Adresse</label>
          ...
          <div className="space-y-6">
            <div className="card p-6">
              <h2 className="text-sm font-semibold text-[#0C0E12] mb-5 pb-4 border-b border-[rgba(12,14,18,0.06)] dark:border-white/10 font-display tracking-tight">Configuration IA</h2>
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-medium text-[#3A3D45] dark:text-[#9CA3AF] mb-1.5">Horaires d'ouverture</label>
                  <input className="input" placeholder="Lun-Ven 9h-19h, Sam 9h-13h" value={form.hours ?? ''} onChange={e => upd('hours', e.target.value)} />
                  <p className="text-[11px] text-[#B0B5C3] dark:text-[#5A5F6B] mt-1">L'IA utilise ces horaires pour répondre aux clients</p>
                </div>
                <div>
                  <label className="block text-xs font-medium text-[#3A3D45] dark:text-[#9CA3AF] mb-1.5">Tarif de consultation</label>
                  <input className="input" placeholder="200 DH" value={form.price ?? ''} onChange={e => upd('price', e.target.value)} />
                  <p className="text-[11px] text-[#B0B5C3] dark:text-[#5A5F6B] mt-1">Affiché automatiquement lors des demandes de tarif</p>
                </div>
              </div>
            </div>

            <div className="card p-6">
              <h2 className="text-sm font-semibold text-[#0C0E12] mb-5 pb-4 border-b border-[rgba(12,14,18,0.06)] dark:border-white/10 font-display tracking-tight">Abonnement</h2>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[#7A7F8E] dark:text-[#9CA3AF]">Plan actuel</span>
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-lg ${planColors[clinic?.plan ?? 'STARTER']}`}>
                    {clinic?.plan}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[#7A7F8E] dark:text-[#9CA3AF]">Statut</span>
                ...
                {clinic?.trialEndsAt && (
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-[#7A7F8E] dark:text-[#9CA3AF]">Essai gratuit</span>
                    <span className="text-sm text-[#0C0E12] dark:text-[#F1F2F4]">jusqu'au {new Date(clinic.trialEndsAt).toLocaleDateString('fr-FR')}</span>
                  </div>
                )}
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[#7A7F8E] dark:text-[#9CA3AF]">Membre depuis</span>
                  <span className="text-sm text-[#0C0E12] dark:text-[#F1F2F4]">{clinic?.createdAt ? new Date(clinic.createdAt).toLocaleDateString('fr-FR') : '—'}</span>
                </div>
              </div>
